...
if __name__ == '__main__':
    ...
    parser.add_argument('--saved_models', type=str,
            help='path to the saved train models')
    ...
    for i in range(args.epochs):
        ...
        #save models
        if train_bkd_acc[0] > best_sel_acc:
            best_sel_acc = train_bkd_acc[0]
            print("Saving sel model...")
            torch.save(model.sel_pred.state_dict(), "retrain_saved_models/sel_models.dump")
        if train_bkd_acc[1] > best_cond_acc:
            best_cond_acc = train_bkd_acc[1]
            print("Saving cond model...")
            torch.save(model.cond_pred.state_dict(), "retrain_saved_models/cond_models.dump")
        if train_bkd_acc[2] > best_group_acc:
            best_group_acc = train_bkd_acc[2]
            print("Saving group model...")
            torch.save(model.group_pred.state_dict(), "retrain_saved_models/group_models.dump")
        if train_bkd_acc[3] > best_order_acc:
            best_order_acc = train_bkd_acc[3]
            print("Saving order model...")
            torch.save(model.order_pred.state_dict(), "retrain_saved_models/order_models.dump")
