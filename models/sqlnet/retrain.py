...
>>>>>>> 09ed1d31ac434a47911dcd6ae485a3ca4468c2cf
    N_word=50
    B_word=6
    GPU = not args.no_gpu
    if args.toy:
...
>>>>>>> 09ed1d31ac434a47911dcd6ae485a3ca4468c2cf
    for i in range(args.epochs):
        print('Retrain Epoch %d @ %s'%(i+1, datetime.datetime.now()))
        print('Retrain Loss = %s'%epoch_train(model, optimizer, BATCH_SIZE, val_sql_data, val_table_data, schemas, TRAIN_ENTRY))
        train_tot_acc, train_bkd_acc = epoch_acc(model, BATCH_SIZE, val_sql_data, val_table_data, schemas, TRAIN_ENTRY, train_flag = True)
        print('Retrain Train acc_qm: %s' % train_tot_acc)
        print('Retrain Breakdown results: sel: %s, cond: %s, group: %s, order: %s'\
        ...
        print('Retrain Dev acc_qm: %s' % val_tot_acc)
        print('Retrain Breakdown results: sel: %s, cond: %s, group: %s, order: %s'\
